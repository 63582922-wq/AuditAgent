...
from app.services.agent.tools import build_tool_handlers, execute_tool_call, tool_schemas
...
def plan_analysis(db: Session, project_id: str, files: List[FileRecord]) -> Dict[str, Any]:
    ...
    handlers = build_tool_handlers(db, files_summary)
    tools = tool_schemas()
    messages: List[Dict[str, Any]] = [
        {
            ...
            "content": (
                ...
                "必须先调用工具了解文件、缺失资料、检索相关记忆，再输出 JSON 计划。"
            ),
        },
        {
            "role": "user",
            "content": (
                ...
                "priority_actions(数组), reasoning(字符串), agent_mode 固定为 agent"
            ),
        },
    ]
